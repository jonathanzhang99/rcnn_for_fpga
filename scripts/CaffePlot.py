...
def main(files):
    ...
    for i, log_file in enumerate(files):
        loss_iterations, losses, loss_averages, accuracy_iterations, accuracies, accuracies_iteration_checkpoints_ind, fileName = parse_log(log_file)
        disp_results(fig, ax1, ax2, loss_iterations, losses, loss_averages, accuracy_iterations, accuracies, accuracies_iteration_checkpoints_ind, fileName, color_ind=i)
    plt.show()
...
def parse_log(log_file):
    ...
    loss_averages = [losses[0]]
    for i in range(1, len(losses)):
        loss_averages.append((loss_averages[i - 1] * i + losses[i]) / (i + 1))
    ...
    return loss_iterations, losses, loss_averages, accuracy_iterations, accuracies, accuracies_iteration_checkpoints_ind, fileName


def disp_results(fig, ax1, ax2, loss_iterations, losses, loss_averages, accuracy_iterations, accuracies, accuracies_iteration_checkpoints_ind, fileName, color_ind=0):
    ...
    if accuracies.size:
       ...
       # print 'Top 4 accuracies:'		
        ...
        plt.title(plotTitle)
    ax1.plot(loss_iterations, losses, color=plt.rcParams['axes.color_cycle'][(color_ind * 3 + 0) % modula], label="losses", linewidth=1)
    ax1.plot(loss_iterations, loss_averages, color=plt.rcParams['axes.color_cycle'][(color_ind * 3 + 1) % modula], label="loss averages", linewidth=3)
    ax2.plot(accuracy_iterations, accuracies, plt.rcParams['axes.color_cycle'][(color_ind * 3 + 2) % modula], label=str(fileName), linewidth=3)
